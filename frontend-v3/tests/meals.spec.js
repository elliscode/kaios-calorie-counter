...
// ─── Real default (no seed) ────────────────────────────────────────────────

test('Meals is on by default', async ({ page }) => {
  await mockDataHost(page, { mealsEnabled: null }); // skip the legacy-off seed, exercise the real default
  await page.goto('/');
...
// ─── Everything else: seeded off, matching every test's own explicit setup ─
// (mockDataHost's default mealsEnabled seed is 'false' precisely so these
// keep testing what they say they're testing, independent of the real
// default above.)

test.describe('Meals settings and behavior', () => {
  ...
  test('Meals off shows dependent rows hidden', async ({ page }) => {
  ...
  test('diary groups entries by meal, in Meals-manager order, with an Other group for unassigned and dangling meal ids', async ({ page }) => {
    ...
    await expect(page.locator('.diary-group-header-name')).toHaveText(['Breakfast', 'Lunch', 'Other']);
    await expect(page.locator('.food-row-name')).toHaveText(['Toast', 'Sandwich', 'Mystery Snack', 'Ghost Meal Item']);
  });

  test('each meal group header shows the summed calories of its own entries, not the whole day', async ({ page }) => {
    await enableMeals(page);
    var today = await page.locator('#input-diary-date').inputValue();

    // seedGuesstimates hardcodes 100 cal/entry — two in Breakfast (200 total),
    // one in Lunch (100) — proving the total is per-group, not a running/day
    // total, and that it's a sum (2×100), not a count.
    await seedGuesstimates(page, today, [
      { name: 'Toast', createdAt: 100, mealId: 'breakfast' },
      { name: 'Eggs', createdAt: 150, mealId: 'breakfast' },
      { name: 'Sandwich', createdAt: 200, mealId: 'lunch' }
    ]);
    ...
    await expect(page.locator('.diary-group-header')).toHaveText(['Breakfast200 cal', 'Lunch100 cal']);
  });
  ...
  test('diary is a flat list with no group headers when Meals is off (regression guard)', async ({ page }) => {
    ...
    await page.reload();
    await expect(page.locator('#panel-diary')).toHaveAttribute('active', 'true');
    await expect(page.locator('.diary-group-header-name')).toHaveCount(0);
  ...
  test('skippable: turning off Require Meal Selection restores instant add (Other group)', async ({ page }) => {
    ...
    await expect(page.locator('#panel-diary')).toHaveAttribute('active', 'true');
    await expect(page.locator('.diary-group-header-name')).toHaveText(['Other']);
    await expect(page.locator('.food-row-name')).toHaveText('Apple, Raw');
  });
  ...
  test('mandatory: Guesstimate requires a meal before it can submit', async ({ page }) => {
    ...
    await expect(page.locator('#panel-diary')).toHaveAttribute('active', 'true');
    await expect(page.locator('.diary-group-header-name')).toHaveText(['Snacks']);
  });
