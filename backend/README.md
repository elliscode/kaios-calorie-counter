...
| `/submit-upc-mapping` | **Anonymous-friendly**, same trust tier as `/submit` — body `{csrf, upc, foodId, foodName, servingName, servingQuantity}`, all required. The end-user-facing sibling of `/admin/add-upc-mapping`: creates a pending UPC mapping directly, no food/serving submission involved, used by the app's "Match this barcode to an existing food" flow when a scanned UPC has no already-known mapping. Goes through the same admin-reviewed queue as everything else before it can ever reach another device via export. |
